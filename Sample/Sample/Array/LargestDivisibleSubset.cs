...
namespace Sample.Array
{
    ...
    {
        public static LargestDivisibleSubset Instance = new LargestDivisibleSubset();
        ...
        private List<int> GetElement(Dictionary<int, List<int>> map, int target)
        {
            List<int> targetList = null;
            foreach(var pair in map)
            {
                var list = pair.Value;
                int divisor = list[list.Count -1];
                if(target % divisor == 0)
                {
                    targetList = (targetList != null && targetList.Count > list.Count) ? targetList : list;
                }
            }
